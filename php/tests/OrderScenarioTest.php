...
use Folio\CodingInterview\Presentation\AdditionalContributionOrderRequest;
use Folio\CodingInterview\Presentation\BadRequestException;
use Folio\CodingInterview\Presentation\GetAssetRequest;
use Folio\CodingInterview\Presentation\MarketPriceItemDto;
use Folio\CodingInterview\Presentation\NewContributionOrderRequest;
use Folio\CodingInterview\Presentation\PortfolioItemDto;
use Folio\CodingInterview\Presentation\RebalanceOrderRequest;
use Folio\CodingInterview\Presentation\UpdateMarketPriceRequest;
...
final class OrderScenarioTest extends TestCase
{
    public function test新規拠出追加拠出リバランスの一連の操作が正しく機能する(): void
    {
        ...
        $mp = $server->marketPriceController;
        ...
        $mp->updateMarketPrice(new UpdateMarketPriceRequest([
            new MarketPriceItemDto('Toyopa', '2.5'),
            new MarketPriceItemDto('Somy', '3.0'),
        ]));
        ...
        $oc->newContributionOrder(new NewContributionOrderRequest($userId, '100000'));
        ...
        $this->assertSame('38000', (string)(new BigDecimal($toyopa1->evaluationAmount)));
        $this->assertSame('57000', (string)(new BigDecimal($somy1->evaluationAmount)));
        $this->assertSame('5000', (string)(new BigDecimal($asset1->cashAmount)));

        // When: 追加拠出を 100,000 円で注文する
        $oc->additionalContributionOrder(new AdditionalContributionOrderRequest($userId, '100000'));
        ...
        $this->assertSame('76000', (string)(new BigDecimal($toyopa2->evaluationAmount)));
        $this->assertSame('114000', (string)(new BigDecimal($somy2->evaluationAmount)));
        ...
        $this->assertSame('19000', (string)(new BigDecimal($toyopa3->evaluationAmount)));
        $this->assertSame('171000', (string)(new BigDecimal($somy3->evaluationAmount)));
